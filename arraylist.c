...
int ArrayList_add( ArrayList *list, void *data ){
    int offset; //Index to add into
    //If list has no items -> Insert at beginning
    if( list->length < 1 ){
        offset = 0;
    }
    else{ //Get last index and insert at end
...
ArrayList* createNewScope(ArrayList * prevList, size_t data_size) {
	ArrayList *newCurrentScope = newArrayList(data_size);

	int tempValue = prevList->nextIndex;
	prevList->next[tempValue] = newCurrentScope;
	

	prevList->nextIndex = prevList->nextIndex + 1;
	
	newCurrentScope->prev = prevList;
